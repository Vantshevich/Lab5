...
//	Удалить из каждой строки, начиная со второй слова, которые встречаются в первой 
//	строке и записать результат  в новый файл Output.txt. Упорядочить слова  в полученных 
//	строках по  возрастанию количества цифр в словах, результат сортировки записать в файл Out_sort.txt.
...
MyString* WordSplitting(MyString& str)
{
	int NumberOfWords = 0;
	...
	MyString* WordMass = new MyString[NumberOfWords + 1];
	int j = 0;
...
MyString& DeleteIfAttendInFirst(MyString* Changing, MyString* First)
{
